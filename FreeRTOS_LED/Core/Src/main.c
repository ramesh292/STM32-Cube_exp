...
int main(void)
{
  ...
  osThreadDef(LED_BLINK, Start_LED, osPriorityNormal, 0, 128);
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
   ...
   */
  //HAL_UART_Receive_IT(&huart2, rx_data_, 6);
      	...
      	if(rx_data[0]=='\r'){
      		 HAL_UART_Transmit(&huart2,newline,2,10);
      		  ...
      		  // no=no*10+(rx_data[0]-'0');

      	   }

//      	if(rx_data[0]=='\x0A') HAL_UART_Transmit(&huart2, rx_data_, 1,10);
//      	if(rx_data[0]=='\0')HAL_UART_Transmit(&huart2, rx_data_, 1,10);
...
//    if(HAL_OK == HAL_UART_Receive_IT(&huart2, rx_data, 10)){
//    	if(rx_data[0]=='\n'){
//    		   k=no;
//    		   no=0;
//    	   }else{
